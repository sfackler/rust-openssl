...
/// HKDF modes of operation. See [`hkdf_mode`](PKeyContext::hkdf_mode)
#[cfg(ossl111)]
pub enum HkdfMode {
    /// Extract followed by expand
    ExtractThenExpand = ffi::EVP_PKEY_HKDEF_MODE_EXTRACT_AND_EXPAND as isize,
    /// Extract only HKDF
    Extract = ffi::EVP_PKEY_HKDEF_MODE_EXTRACT_ONLY as isize,
    /// Expand only HKDF
    Expand = ffi::EVP_PKEY_HKDEF_MODE_EXPAND_ONLY as isize,
}
...
impl<'a> PKeyContext<'a> {
    ...
    pub fn new_pkey<T>(key: &'a PKeyRef<T>) -> Result<PKeyContext<'a>, ErrorStack>
    where
        T: HasPrivate,
    {
        unsafe {
    ...
    /// Creates a new `PKeyContext` in HKDF derivation mode using the digest provided.
    ///
    /// This corresponds to [`EVP_PKEY_CTX_new_id`] using `EVP_PKEY_HKDF` followed by
    /// [`EVP_PKEY_derive_init`].
    ///
    /// [`EVP_PKEY_CTX_new_id`]: https://www.openssl.org/docs/man1.1.1/man3/EVP_PKEY_CTX_new_id.html
    /// [`EVP_PKEY_derive_init`]: https://www.openssl.org/docs/man1.1.1/man3/EVP_PKEY_derive_init.html
    #[cfg(ossl110)]
    pub fn new_hkdf() -> Result<Self, ErrorStack> {
        unsafe {
            ...
            .map(|p| PKeyContext(p, PhantomData))
            .and_then(|p| cvt(ffi::EVP_PKEY_derive_init(p.0)).map(|_| p))
        }
    }
    ...
    pub fn set_hkdf_mode(&mut self, mode: HkdfMode) -> Result<(), ErrorStack> {
        unsafe {
            cvt(ffi::EVP_PKEY_CTX_set_hkdf_mode(self.0, mode as c_int))?;
        }
...
pub fn hkdf_expand(
    ...
    let mut ctx = PKeyContext::new_hkdf()?;
...
mod test {
    ...
    fn derive_without_peer() {
        ...
        let mut ctx = PKeyContext::new_pkey(&pkey).unwrap();
    ...
    fn test_large_hkdf_info() {
        ...
        let mut hkdf = PKeyContext::new_hkdf().unwrap();
