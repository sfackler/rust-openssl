...
impl PKey<Private> {
    ...
    /// Creates a new `OsslParam` from `PKey<Private>`.
    #[corresponds(EVP_PKEY_todata)]
    #[cfg(ossl300)]
    pub fn todata(&self) -> Result<&OsslParamRef, ErrorStack> {
        unsafe {
            let mut params: *mut ffi::OSSL_PARAM = ptr::null_mut();
            // The EVP_PKEY_KEYPAIR is needed for ECDSA key as it can not export only private key
...
impl PKey<Public> {
    ...
    /// Creates a new `OsslParam` from `PKey<Public>`.
    #[corresponds(EVP_PKEY_todata)]
    #[cfg(ossl300)]
    pub fn todata(&self) -> Result<&OsslParamRef, ErrorStack> {
        unsafe {
...
mod tests {
    ...
    fn test_build_pkey_rsa() {
        ...
        /* Now, construct the OSSL_PARAMS manually from the old key */
        ...
        let params = pkey2.todata().unwrap();
