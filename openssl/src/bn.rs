...
mod tests {
    ...
    fn test_exponentiation_time() {
        // Checks the time taken to do `(((g^x_1 mod n)^x_2 mod n)^x_3 mod n)...` using `mod_exp_into_self`
        ...
        let base_str = "";
        let mut accumulator = BigNum::from_dec_str(&base_str).unwrap();

        // Number of primes to exponenetiate
        ...
        println!("Generating {} primes takes {} seconds", num_exponentiations, start.elapsed().as_secs());

        let mut bn_ctx = BigNumContext::new().unwrap();

        let start = Instant::now();
        for i in 0..num_exponentiations {
            accumulator.mod_exp_into_self(&primes[i], &modulus, &mut bn_ctx).unwrap();
        }
        println!("mod_exp_into_self: time to exponentiate {} primes is {:?}", num_exponentiations, start.elapsed().as_secs());
