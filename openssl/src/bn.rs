...
mod tests {
    ...
    fn test_exponentiation_time() {
        // Checks the time taken to do `(((g^x_1 mod n)^x_2 mod n)^x_3 mod n)...` using `mod_exp_mut`
        ...
        let end_since_the_epoch = SystemTime::now().duration_since(UNIX_EPOCH)
            .expect("Time went backwards");
        println!("Generating {} primes takes {} seconds", num_exponentiations, end_since_the_epoch.as_secs()-start_since_the_epoch.as_secs());
        ...
        let end_since_the_epoch = SystemTime::now().duration_since(UNIX_EPOCH)
            .expect("Time went backwards");
        println!("Time to exponentiate {} primes is {:?}", num_exponentiations, end_since_the_epoch.as_secs()-start_since_the_epoch.as_secs());
    }
}
