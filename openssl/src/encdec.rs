...
impl DecoderCtxRef {
    ...
    #[corresponds(OSSL_DECODER_CTX_set_passphrase)]
    #[allow(dead_code)]
    fn set_passphrase_callback<F: FnOnce(&mut [u8]) -> Result<usize, ErrorStack>>(
        &mut self,
        callback: F,
    ) -> Result<(), ErrorStack> {
        let mut callback = CallbackState::new(callback);
        cvt(unsafe {
...
impl<'a, T: SelectionT> Decoder<'a, T> {
    ...
    pub fn context(self, pkey_ptr: *mut *mut ffi::EVP_PKEY) -> Result<DecoderCtx, ErrorStack> {
        DecoderCtx::new_for_key(
            pkey_ptr,
            T::SELECTION,
            self.format,
            self.structure,
    ...
    pub fn decode(self, data: &[u8]) -> Result<PKey<T>, ErrorStack> {
        ...
        let mut ctx = self.context(&mut pkey_ptr)?;
...
impl EncoderCtxRef {
    ...
    #[allow(dead_code)]
    fn set_cipher(&mut self, cipher: Option<&CipherRef>) -> Result<(), ErrorStack> {
...
impl<'a> Encoder<'a> {
    ...
    #[allow(dead_code)]
    pub fn set_cipher(mut self, cipher: &'a CipherRef) -> Self {
    ...
    mod decoder {
        ...
        mod private {
            ...
            fn test_rsa_pem_callback() {
                ...
                let mut pkey_ptr = ptr::null_mut();
                let mut ctx = Decoder::<Private>::new()
                    .set_key_type(Id::RSA)
                    .set_format(KeyFormat::Pem)
                    .set_structure(Structure::PKCS1)
                    .context(&mut pkey_ptr)
                    .unwrap();
                ctx.set_passphrase_callback(|password| {
                    password_queried = true;
                    password[..6].copy_from_slice(b"mypass");
                    Ok(6)
                })
                .unwrap();
                ctx.decode(include_bytes!("../test/rsa-encrypted.pem"))
                    .unwrap();
                assert!(password_queried);
                unsafe { PKey::<Private>::from_ptr(pkey_ptr) }
                    .rsa()
                    .unwrap();
            }
    ...
    mod encoder {
        ...
        mod params {
            ...
            fn test_dh_pem() {
                ...
                assert_eq!(
                    pem_key.prime_p().to_vec(),
                    pkey.dh().unwrap().prime_p().to_vec(),
                );
            }
        ...
        mod public {
            ...
            fn test_rsa_pem() {
                ...
                let pem_str = from_utf8(&pem).unwrap();
                assert_eq!(pem, expected, "{pem_str}");
            }
            ...
            fn test_rsa_der_pkcs1() {
                ...
                assert_eq!(der_key.n().to_vec(), pkey.rsa().unwrap().n().to_vec());
            }
        }
        ...
        mod public_from_private {
            ...
            fn test_rsa_pem() {
                ...
                assert_eq!(pem_key.n().to_vec(), pkey.rsa().unwrap().n().to_vec());
            }
        ...
        mod private {
            ...
            fn test_rsa_pem_encrypted() {
                ...
                assert_eq!(pkey2.n().to_vec(), pkey.rsa().unwrap().n().to_vec());
            }
