...
impl<'a, 'b> ParamBuilder<'a> {
    ...
    pub fn push_utf8_string(&mut self, key: &'b CStr, string: &'a str) -> Result<(), ErrorStack> {
    ...
    pub fn push_byte_string(&mut self, key: &'b CStr, value: &'a [u8]) -> Result<(), ErrorStack> {
        cvt(unsafe {
...
mod test {
    ...
    fn test_param_builder_string() {
        let mut builder = ParamBuilder::new();
        builder
            .push_utf8_string(c_str(b"group\0"), "primve256v1")
            .unwrap();
        let params = builder.build().unwrap();
