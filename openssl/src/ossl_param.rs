...
impl OsslParamArray {
    ...
    /// in the `OsslParamArray` array and returns a reference to it.
    #[corresponds(OSSL_PARAM_locate)]
    #[allow(dead_code)] // TODO: remove when when used by ML-DSA / ML-KEM
    pub(crate) fn locate<'a>(&'a self, key: &CStr) -> Result<OsslParam<'a>, ErrorStack> {
        unsafe {
...
impl<'a> OsslParam<'a> {
    ...
    unsafe fn from_ptr(ptr: *const ffi::OSSL_PARAM) -> OsslParam<'a> {
        OsslParam(&*ptr)
    }
    ...
    #[allow(dead_code)] // TODO: remove when when used by ML-DSA / ML-KEM
    pub(crate) fn get_utf8_string(&self) -> Result<&'a str, ErrorStack> {
        unsafe {
    ...
    #[allow(dead_code)] // TODO: remove when when used by ML-DSA / ML-KEM
    pub(crate) fn get_octet_string(&self) -> Result<&'a [u8], ErrorStack> {
        unsafe {
