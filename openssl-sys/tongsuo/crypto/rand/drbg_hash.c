...
{
    return hash_df(drbg, out, in_byte, in1, in1len, NULL, 0,
                   NULL, 0);
}
                            ...
                            const unsigned char *adin, size_t adin_len)
{
    ...
    if (drbg->type == NID_sm3) {
        /* GM/T 0105-2021 B.5
         * a) SM3_df(0x01 || entropy_input || V || additional_input) */
        if (!hash_df(drbg, hash->C, 0x01, ent, ent_len,
                     hash->V, drbg->seedlen, adin, adin_len))
            return 0;
    } else {
        /* (Step 1-2) V = Hash_df(0x01 || V || entropy_input || additional_input) */
        /* V about to be updated so use C as output instead */
        if (!hash_df(drbg, hash->C, 0x01, hash->V, drbg->seedlen,
...
int drbg_hash_init(RAND_DRBG *drbg)
{
    ...
    case NID_sha512:
        ...
        break;
    case NID_sha512_224:
        hash->md = EVP_sha512_224();
        break;
    case NID_sha512_256:
        hash->md = EVP_sha512_256();
        break;
    ...
    hash->blocklen = EVP_MD_size(hash->md);
    ...
    /* GM/T 0105-2021 B.1 maximum of requested_number_of_bits = 256 bits */
    if (drbg->type == NID_sm3)
